/**
 * @file driver_laplacian_gemslr_seq.cpp
 * @brief Example file, use sequential fgmres + gemslr to solve real laplacian.
 */
...
int main (int argc, char *argv[]) 
{
   ...
   SequentialVectorClass<double> x, b;
   
   /* the sequential CSR matrix template */
   CsrMatrixClass<double> csr_mat;
   
   /* The FlexGMRES solver template, <Matrixtype, Vectortype, Datatype> */
   FlexGmresClass<CsrMatrixClass<double>, SequentialVectorClass<double>, double> solver;
   
   /* The GemslrClass solver template, <Matrixtype, Vectortype, Datatype> */
   GemslrClass<CsrMatrixClass<double>, SequentialVectorClass<double>, double> precond;
   
   /*--------------------------
    * 1. Read Laplacian Parameters
    *--------------------------*/
   
   if(read_double_laplacian_param(nmats, &nx, &ny, &nz, &shift, &alphax, &alphay, &alphaz) != 0)
   {
   ...
   for(i = 0 ; i < nmats ; i ++)
   {
      ...
      if(rand_perturb)
      {
         PARGEMSLR_PRINT("\tSolving Laplacian %d %d %d %lf %lf %lf %lf with random diagonal pertubation\n",nx[i],ny[i],nz[i],alphax[i],alphay[i],alphaz[i],shift[i]);
      }
      else
      {
