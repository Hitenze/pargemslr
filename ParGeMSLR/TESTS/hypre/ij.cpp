      ...
      char *argv[] )
{
   ...
   {
      if ( strcmp(argv[arg_index], "-gemslrfile") == 0 )
      {
         arg_index++;
         gemslr_input_file = argv[arg_index];
      }
   ...
   if ( print_usage )
   {
      if ( myid == 0 )
      {
         ...
         hypre_printf("\n");
         hypre_printf("  -gemslrfile <filename>     : ");
         hypre_printf("read gemslr setup from file (default is ./inputs)\n");
         ...
         hypre_printf("    -shift <val>         : The diagonal shift A - shift*I\n");
         ...
         hypre_printf("       83=GEMSLR-FlexGMRES  \n");
   ...
   if (solver_id == -1)
   {
      ...
      for (i = 0; i < nmv; i++)
      {
         HYPRE_ParCSRMatrixMatvec(0.01, parcsr_A, x, 1.01, b);
      }
 ...
 *-----------------------------------------------------------*/
   ...
   if (solver_id == 60 || solver_id == 61 || solver_id == 72 || solver_id == 82 || solver_id == 83 || solver_id == 47)
   {
      ...
      else if (solver_id == 83)
      {
         ...
         /* create precon */
         HYPRE_GEMSLRCreateFromFile(&pcg_precond, gemslr_input_file);
                   ...
                   HYPRE_ParCSRMatrix  *A_ptr     )
{
   ...
   while (arg_index < argc)
   {
      ...
      else if ( strcmp(argv[arg_index], "-shift") == 0 )
      {
         arg_index++;
         shift = atof(argv[arg_index++]);
      }
      else if ( strcmp(argv[arg_index], "-perturb") == 0 )
      {
         arg_index++;
         perturb = atof(argv[arg_index++]);
      }
   ...
   if (num_fun == 1)
   {
      ...
      if(perturb!=0.0)
      {
         ...
         int pi, pi1, pi2, pj, pcol;
         int pn = hypre_CSRMatrixNumRows(A_diag);
         
         for(pi = 0 ; pi < pn ; pi ++)
         {
            pi1 = A_diag_i[pi];
            pi2 = A_diag_i[pi+1];
            for(pj = pi1 ; pj < pi2 ; pj ++)
            {
               pcol = A_diag_j[pj];
               if(pcol == pi)
               {
                  A_diag_data[pj] -= perturb * ((HYPRE_Real)rand()/(HYPRE_Real)RAND_MAX);
               }
            }
         }
