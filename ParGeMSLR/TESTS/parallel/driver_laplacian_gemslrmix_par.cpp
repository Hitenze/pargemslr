/**
 * @file driver_laplacian_gemslrmix_par.cpp
 * @brief Example file, use parallel fgmres + gemslr. The solver is double precision while the preconditioner is single precision.
 */
...
int main (int argc, char *argv[]) 
{
   ...
   /*---------create precond */
   ParallelMixPrecondClass *precondp = new ParallelMixPrecondClass();
   
   ParallelGemslrClass<ParallelCsrMatrixClass<float>, ParallelVectorClass<float>, float> *precondfp = 
      new ParallelGemslrClass<ParallelCsrMatrixClass<float>, ParallelVectorClass<float>, float>();
   ...
   if(PargemslrReadInputArg("lapfile", lapfile, argc, argv))
   {
      if(myid == 0)
      {
         printf("Reading laplacian from file %s\n",lapfile);
      }
   ...
   else
   {
      if(myid == 0)
      {
         printf("Reading laplacian from file \"lapfile_real\"\n");
      }
