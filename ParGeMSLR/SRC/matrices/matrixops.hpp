...
namespace pargemslr
{
   ...
   /**
    * @brief   Compute the transpose of a dense matrix. Typically not recommended unless this operation can significantly improve the performance.
    * @details Compute the transpose of a dense matrix. Typically not recommended unless this operation can significantly improve the performance.
    ...
    */
   template <class VectorType, class MatrixType, typename DataType, typename RealDataType>
   int PargemslrArnoldiNoRestart2( MatrixType &A, int mstart, int msteps, DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType tol_orth, RealDataType tol_reorth, DataType shift, int &nmvs);
   
#ifdef PARGEMSLR_PARPACK
 
   /**
    * @brief   The implicit restarted Arnoldi using PARPACK.
    * @details The implicit restarted Arnoldi using PARPACK.
    * @note    Template function, matrix requires SetupVectorPtrStr, GetNumRowsLocal, MatVec, and GetDataLocation functions.
    * @param [in]       A The target matrix with Matvec and GetNumRows function.
    * @param [in]       msteps (Maximum) Number of columns in the working matrix V.
    * @param [in]       maxits (Maximum) Number of restarts.
    * @param [in]       nev Target number of eigenvalues.
    * @param [in]       which Char of length 2, target eigenvalues.
    * @param [in]       tol_eig Tolerance for eigenvalues.
    * @param [out]      V The basis vectors generated by Arnoldi.
    * @param [out]      H The upper Hessenberg matrix generated by Arnoldi.
    * @param [out]      nmvs Number of matrix vector products applied.
    * @return           # of eigenvalues we computed.                                                                                                                                                       
    */
   template <class VectorType, class MatrixType>
   int ArnoldiImplicitRestartHost( MatrixType &A, int msteps, int maxits, int nev, char *which, float tol_eig, matrix_dense_float &V, matrix_dense_float &H, int &nmvs);
    ...
    */
   template <class VectorType, class MatrixType>
   int ArnoldiImplicitRestartHost( MatrixType &A, int msteps, int maxits, int nev, char *which, double tol_eig, matrix_dense_double &V, matrix_dense_double &H, int &nmvs);
    ...
    */
   template <class VectorType, class MatrixType>
   int ArnoldiImplicitRestartHost( MatrixType &A, int msteps, int maxits, int nev, char *which, float tol_eig, matrix_dense_complexs &V, matrix_dense_complexs &H, int &nmvs);
    ...
    */
   template <class VectorType, class MatrixType>
   int ArnoldiImplicitRestartHost( MatrixType &A, int msteps, int maxits, int nev, char *which, double tol_eig, matrix_dense_complexd &V, matrix_dense_complexd &H, int &nmvs);
    ...
    * @param [in]       opt* The option of length 2. 1st could be 'S' (smallest), 'L' (largest); 2nd could be 'R' (real part), 'I' (imag part), 'M' (magnitude).
    * @param [in,out]   V Matrix holding the Krylov basis, space should be reserved, V(:,0:mstart) should be set already.
    * @param [in,out]   H The upper Hessenberg matrix generated by Arnoldi, space should be reserved.
    * @param [in]       tol_orth the tolorance for the Arnoldi.
    * @param [in]       tol_reorth the tolorance for the reorthgonation, set to -1.0 to turn off reorthgonation.
    ...
    */
   template <class VectorType, class MatrixType, typename DataType, typename RealDataType>
   int PargemslrArnoldiThickRestartNoLock( MatrixType &A, int msteps, int maxits, int rank, int rank2, RealDataType tr_fact, RealDataType tol_eig, char* opt, DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType tol_orth, RealDataType tol_reorth, int &nmvs);
    ...
    */
   template <class VectorType, class MatrixType, typename DataType, typename RealDataType>
   int PargemslrArnoldiThickRestartNoLock( MatrixType &A, int msteps, int maxits, int rank, int rank2, RealDataType tr_fact, RealDataType tol_eig, RealDataType (*weight)(ComplexValueClass<RealDataType>), DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType tol_orth, RealDataType tol_reorth, int &nmvs);
    ...
    */
   template <typename T>
   int PargemslrArnoldiThickRestartChooseEigenValuesReal( DenseMatrixClass<T> &H, DenseMatrixClass<T> &Q, T h_last, T (*weight)(ComplexValueClass<T>), T truncate, int &ncov, int &nicov, int &nsatis, T tol_eig, bool &cut, SequentialVectorClass<T> &wr, SequentialVectorClass<T> &wi, vector_int &icov, vector_int &iicov, vector_int &work_int, SequentialVectorClass<T> &dcov, SequentialVectorClass<T> &dicov, SequentialVectorClass<T> &dall);
   ...
   int PargemslrArnoldiThickRestartChooseEigenValuesComplex( DenseMatrixClass<DataType> &H, DenseMatrixClass<DataType> &Q, DataType h_last, RealDataType (*weight)(DataType), RealDataType truncate, int &ncov, int &nicov, int &nsatis, RealDataType tol_eig, bool &cut, SequentialVectorClass<DataType> &w, vector_int &icov, vector_int &iicov, vector_int &work_int, SequentialVectorClass<RealDataType> &dcov, SequentialVectorClass<RealDataType> &dicov, SequentialVectorClass<RealDataType> &dall);
