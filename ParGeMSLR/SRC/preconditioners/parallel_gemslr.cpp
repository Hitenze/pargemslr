...
namespace pargemslr
{
      ...
      if(str._B_solver)
      {
         ...
         switch(str._B_solver[0]->GetSolverType())
         {
            ...
            case kSolverPoly:
            {
               ...
               for(i = 0 ; i < this->_ncomps ; i ++)
               {
                  PARGEMSLR_PLACEMENT_NEW( this->_B_solver[i], kMemoryHost, PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>);
                  
                  PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType> &poly1 = *(PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*) str._B_solver[i];
                  PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType> &poly2 = *(PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*) this->_B_solver[i];
   ...
   int ParallelGemslrClass<MatrixType, VectorType, DataType>::CheckParameters()
   {
      ...
      /* setup diagonal shift */
      /* this is the standard MILU with shift, not in use yet
      if(this->_gemslr_setups._solve_option_setup == kGemslrMmulSolve)
      {
         ...
         VectorType ones, b;
         DataType one, zero;
         RealDataType b_min, b_min_global;
         ...
         ones.Setup( n_local,  this->_matrix->GetRowStartGlobal(),  this->_matrix->GetNumRowsGlobal(), false, *this->_matrix);
         b.Setup( n_local,  this->_matrix->GetRowStartGlobal(),  this->_matrix->GetNumRowsGlobal(), true, *this->_matrix);
         
         ones.Fill(DataType(1.0));
         this->_matrix->MatVec( 'N', one, ones, zero, b);
         
         SequentialVectorClass<RealDataType> b_real;
         
         b_real.Setup(n_local);
         
         for(i = 0 ; i < n_local ; i ++)
         {
            b_real[i] = PargemslrReal(b[i]);
         }
         
         b_min = b_real.Min();
         PARGEMSLR_MPI_CALL( PargemslrMpiAllreduce( &b_min, &b_min_global, 1, MPI_MIN, comm) );
         
         if(b_min_global < RealDataType())
         {
            this->_gemslr_setups._diag_shift_milu = -b_min_global;
            //if(myid == 0)
            //{
            //   cout<<"Adding diagonal shift of "<<this->_gemslr_setups._diag_shift_milu<<endl;
            //}
         }
         else
         {
            this->_gemslr_setups._diag_shift_milu = 0;
         }
...
perm_gemslr_global:
   ...
   int ParallelGemslrClass<MatrixType, VectorType, DataType>::SetupBSolveILUT( VectorType &x, VectorType &rhs, int level)
   {
       ...
       */
      if(this->_gemslr_setups._solve_option_setup != kGemslrMmulSolve || level == this->_nlev_used - 1)
      {
...
#pragma omp parallel for private(i) PARGEMSLR_OPENMP_SCHEDULE_DEFAULT
      ...
      {
         switch(this->_gemslr_setups._level_setups._B_smooth_option1)
         {
            case kGemslrBSmoothBMILU:
            {
               /* in this case, do nothing, since we use MILU as smoother */
               break;
            }
            case kGemslrBSmoothBILU:
            {
               /* start building the ILU on this level */
               PARGEMSLR_MALLOC( level_str._B_solver, ncomp, kMemoryHost, SolverClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*);
...
#pragma omp parallel for private(i) PARGEMSLR_OPENMP_SCHEDULE_DEFAULT
            ...
            case kGemslrBSmoothAMILU:
            {
               PARGEMSLR_ERROR("Unsupported GeMSLR B smoothing option.");
               return PARGEMSLR_ERROR_INVALED_OPTION;
               break;
            }
            case kGemslrBSmoothAILU:
            {
               PARGEMSLR_ERROR("Unsupported GeMSLR B smoothing option.");
               return PARGEMSLR_ERROR_INVALED_OPTION;
               break;
            ...
            default:
            {
               PARGEMSLR_ERROR("Unknown GeMSLR B smoothing option.");
               return PARGEMSLR_ERROR_INVALED_OPTION;
               break;
            }
         }
      ...
      /* Set up the smoother, put into B_precond */
      if(this->_gemslr_setups._solve_option_setup == kGemslrMmulSolve && level != this->_nlev_used - 1)
      {
         /* start building the modified ILU on this level */
         PARGEMSLR_MALLOC( level_str._B_precond, ncomp, kMemoryHost, SolverClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*);
         
         /* setup the solver 
          * We want LU * 1 = - F * 1
          * Note that B has block diagonal structure.
          */
          
         
         ParallelVectorClass<T>     ones, fones, luones;
         SequentialVectorClass<T>   luones_loc, fones_loc;
         int                        n_start, n1, n2;
         T                          one, zero, mone;
         one = 1.0;
         zero = 0.0;
         ...
         ones.Setup(level_str._F_mat.GetNumColsLocal(),
                     level_str._F_mat.GetColStartGlobal(),
                     level_str._F_mat.GetNumColsGlobal(), 
                     false, 
                     level_str._F_mat);
         
         fones.Setup(level_str._F_mat.GetNumRowsLocal(),
                     level_str._F_mat.GetRowStartGlobal(),
                     level_str._F_mat.GetNumRowsGlobal(), 
                     true, 
                     level_str._F_mat);
         
         luones.Setup(level_str._F_mat.GetNumRowsLocal(),
                     level_str._F_mat.GetRowStartGlobal(),
                     level_str._F_mat.GetNumRowsGlobal(), 
                     true, 
                     level_str._F_mat);
         ...
         level_str._F_mat.MatVec( 'N', mone, ones, zero, fones);
...
#pragma omp parallel for private(i) PARGEMSLR_OPENMP_SCHEDULE_DEFAULT
         ...
         {
            /* build ILU solver */
            PARGEMSLR_PLACEMENT_NEW( level_str._B_precond[i], kMemoryHost, IluClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>);
            IluClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType> *ilup 
                        = (IluClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*) level_str._B_precond[i];
            ...
            /* modified ILU */
            ilup->SetModified(true);
            ilup->SetModifiedShift(this->_gemslr_setups._diag_shift_milu);
            
            /* now setup the modified vector such that LU*ones_loc = fones_loc */
            n1 = this->_dom_ptr_v2[level][i];
            n2 = this->_dom_ptr_v2[level][i+1];
            
            luones_loc.SetupPtr(luones.GetData() + n1 - n_start, n2 - n1, kMemoryHost);
            fones_loc.SetupPtr(fones.GetData() + n1 - n_start, n2 - n1, kMemoryHost);
            
            //level_str._B_mat_v[i].MatVec( 'N', one, luones_loc, zero, fones_loc);
            
            ilup->Setup(luones_loc, fones_loc);
            //ilup->Setup(dummyx, dummyrhs);
   ...
   int ParallelGemslrClass<MatrixType, VectorType, DataType>::SetupBSolveILUK( VectorType &x, VectorType &rhs, int level)
   {
      ...
      /* for MILU, except for the last level, we use MILU as smoother */
      if(this->_gemslr_setups._solve_option_setup != kGemslrMmulSolve || level == this->_nlev_used - 1)
      {
         /* start building the ILU on this level */
         PARGEMSLR_MALLOC( level_str._B_solver, ncomp, kMemoryHost, SolverClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*);
...
#pragma omp parallel for private(i) PARGEMSLR_OPENMP_SCHEDULE_DEFAULT
      ...
      else
      {
         switch(this->_gemslr_setups._level_setups._B_smooth_option1)
         {
            ...
            case kGemslrBSmoothBILU:
            {
               /* in this case, setup the B solve. */
               /* start building the ILU on this level */
               PARGEMSLR_MALLOC( level_str._B_solver, ncomp, kMemoryHost, SolverClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*);
...
#pragma omp parallel for private(i) PARGEMSLR_OPENMP_SCHEDULE_DEFAULT
   ...
   template <class MatrixType, class VectorType, typename DataType>
   int ParallelGemslrClass<MatrixType, VectorType, DataType>::SetupBSolvePoly( VectorType &x, VectorType &rhs, int level)
   {
...
#pragma omp parallel for private(i) PARGEMSLR_OPENMP_SCHEDULE_DEFAULT
      ...
      {
         /* build ILU solver */
         PARGEMSLR_PLACEMENT_NEW( level_str._B_solver[i], kMemoryHost, PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>);
         PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType> *polyp 
                     = (PolyClass<CsrMatrixClass<T>, SequentialVectorClass<T>, DataType>*) level_str._B_solver[i];
   ...
   template int precond_gemslr_csr_par_float::SetupBSolvePoly( ParallelVectorClass<float> &x, ParallelVectorClass<float> &rhs, int level);
   template int precond_gemslr_csr_par_double::SetupBSolvePoly( ParallelVectorClass<double> &x, ParallelVectorClass<double> &rhs, int level);
   template int precond_gemslr_csr_par_complexs::SetupBSolvePoly( ParallelVectorClass<complexs> &x, ParallelVectorClass<complexs> &rhs, int level);
   template int precond_gemslr_csr_par_complexd::SetupBSolvePoly( ParallelVectorClass<complexd> &x, ParallelVectorClass<complexd> &rhs, int level);
...
#pragma omp parallel for private(i) PARGEMSLR_OPENMP_SCHEDULE_DEFAULT
   ...
   int ParallelGemslrClass<MatrixType, VectorType, DataType>::SetupLowRankThickRestartNoLock( ParallelVectorClass<DataType> &x, ParallelVectorClass<DataType> &rhs, int level, int &nmvs, int option)
   {
      ...
      ArnoldiMatrixClass<VectorType, DataType> &temp_EBFC = level_str._EBFC;
      m = PargemslrArnoldiThickRestartNoLock<VectorType>( temp_EBFC, lr_m, maxits, rank2, rank, RealDataType(0.0), tr_fact, tol_eig, &(ParallelGemslrClass<MatrixType, VectorType, DataType>::ComputeDistance), V, H, tol_orth, tol_reorth, nmvs);
   ...
   int ParallelGemslrClass<MatrixType, VectorType, DataType>::SolveLevelGemslr( VectorType &x_out, VectorType &rhs_in, int level, bool doperm)
   {
      ...
      if(level == this->_nlev_used - 1)
      {
         ...
         this->_gemslr_setups._level_setups._C_solve_option == kGemslrCSolveBJILUK))
         {
             ...
             * for PCLR E is -C_offd
             * for PSLR E is -C_offd+FB^{−1}E
             */
            if(level_str._lrc > 0)
            {
               /* apply the low-rank update */
   ...
   int ParallelGemslrClass<MatrixType, VectorType, DataType>::SolveLevelEsmslr( VectorType &x_out, VectorType &rhs_in, int level, bool doperm)
   {
      ...
      if(level == this->_nlev_used - 1)
      {
         ...
         this->_gemslr_setups._level_setups._C_solve_option == kGemslrCSolveBJILUK))
         {
             ...
             * for PCLR E is -C_offd
             * for PSLR E is -C_offd+FB^{−1}E
             * with Eschur this is C_diag-C+EB^{-1}F = C_diag-S
             */
            if(level_str._lrc > 0)
            {
               /* apply the low-rank update */
